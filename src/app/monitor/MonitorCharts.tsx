...
export function MonitorCharts({ runs, allTools }: { runs: RunData[]; allTools: string[] }) {
  ...
  const successRateData = runs.map((r) => ({
    ran_at: r.ran_at,
        ...
        <div className="p-5">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={runs} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="tool_calls" radius={[3, 3, 0, 0]} name="Tool Calls">
                {runs.map((r, i) => (
                  <Cell key={i} fill={r.status === 'error' ? '#f85149' : '#1f6feb'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="p-5">
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={runs} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="items_flagged" stroke="#d29922" strokeWidth={2} dot={{ fill: '#d29922', r: 3 }} name="Items Flagged" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
          ...
          <div className="p-5">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={runs} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                {allTools.map((tool, i) => (
                  <Bar key={tool} dataKey={`tool_breakdown.${tool}`} stackId="tools" fill={TOOL_COLORS[i % TOOL_COLORS.length]} name={tool} radius={i === allTools.length - 1 ? [3, 3, 0, 0] : [0, 0, 0, 0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </div>
